    ...
    <div class="my-5 grid grid-cols-1 md:grid-cols-3">
        <div class="col-span-1 w-full md:border-r-4 md:border-r-transparent mb-5" >
            ...
            <!--<p :class="{'text-2xl': true, 'px-5': true, 'py-2': true, 'text-white': true, 'hover:bg-slate-800': true, 'border-4': true, 'border-transparent': true, 'hover:text-teal-300': true, 'cursor-pointer': true, 'select-none': true, 'text-teal-300': state.page === key, 'bg-slate-800': state.page === key, 'border-cyan-500': state.page === key}" v-for="(value, key) in SettingsList" :key="key" @click="state.page = key">
              {{ value }}</p>-->
            <div :class="{'text-2xl': true, 'px-5': true, 'py-2': true, 'border-4': true, 'cursor-pointer': true, 'select-none': true, 'hover:bg-slate-100': true, 'dark:hover:bg-slate-800': true, 'dark:text-white': true, 'border-[#E60012]': Platform === 'ns', 'border-[#0070D1]': Platform === 'ps', 'border-[#107C10]': Platform === 'xbox'}">Settings</div>
        </div>
        <div class="md:col-start-2 md:col-span-2 w-full md:border-l-4 md:border-l-transparent px-0 md:px-10">
            <div class="grid grid-cols-10">
                <div class="col-span-2 px-0 sm:px-2 md:px-0 xl:px-5">
                    <div v-if="!userInfo.avatar" :class="{'aspect-square': true, 'w-full': true, 'bg-slate-300': true, 'dark:bg-slate-700': true, 'animate-pulse': screenName && !userInfo.avatar, 'rounded-xl': true,}"/>
                    <img v-else class="aspect-square w-full bg-slate-300 dark:bg-slate-700 rounded-xl" :src="mediaPath + userInfo.avatar" :alt="userInfo.display_name" />
                </div>
                <div class="col-span-8 px-5 divide-gray-900 flex flex-col justify-between dark:text-white">
                    <div v-if="!state.editMode">
                        <div class="flex justify-between items-center">
                            <span v-if="screenName" :class="{'text-2xl': true, 'animate-pulse': !userInfo.display_name, 'bg-slate-300': !userInfo.display_name, 'dark:bg-slate-700': !userInfo.display_name, 'h-6': !userInfo.screen_name, 'w-32': !userInfo.display_name, 'rounded-xl': !userInfo.display_name, }">{{ userInfo.display_name ? userInfo.display_name : '' }}</span>
                            <span v-else class="text-2xl ">Twitter username</span>
                            <div class="cursor-pointer hover:bg-slate-200 dark:hover:bg-gray-800 p-3 rounded-full" @click="state.editMode = !state.editMode">
                        ...
                        <div v-if="screenName" :class="{'text-sm': true, 'animate-pulse': !userInfo.screen_name, 'bg-slate-300': !userInfo.screen_name, 'dark:bg-slate-700': !userInfo.screen_name, 'h-5': !userInfo.screen_name, 'w-15': !userInfo.screen_name, 'rounded-xl': !userInfo.display_name, }">{{ userInfo.screen_name ? '@' + userInfo.screen_name : '' }}</div>
                    </div>
                    <div v-else class="items-center gap-2 flex">
                        <div class="basis-10/12">
                            <input id="input-screen-name" type="text" class="form-input w-full rounded-xl h-[2em] md:h-[2.5em] dark:bg-black " v-model="screenName" placeholder="Screen Name">
                        </div>
                        <div class="cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-800 p-2 rounded-full" @click="screenName = ''">
                        ...
                        <div class="cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-800 p-2 rounded-full" @click="state.editMode = !state.editMode">
                    ...
                    <label for="input-screen-name" class="text-red-500 mt-1 px-1" v-if="state.invalidName">Invalid name!</label>
            ...
            <div class="px-0 md:px-5">
                <h4 class="font-bold text-xl dark:text-gray-200 mb-5">Filter tags</h4>
                <input id="input-hashtags-count" type="number" min="0" class="dark:text-white form-input w-full rounded-xl h-[2em] md:h-[2.5em] dark:bg-black " v-model="hashtagsCount" placeholder="Count">
                <label for="input-hashtags-count" class="mb-3 dark:text-gray-300">The images of tweets that exceed the number of hashtags will be filtered out from the public timeline, but there is no restriction on personal timelines.</label>
            </div>
            <hr class="my-5" />
            <div class="flex justify-between px-0 md:px-5">
                <span class="font-bold text-xl dark:text-gray-200">Dark mode</span>
                <dark-button class="h-9 w-9 "  />
            </div>
        </div>
    </div>
</template>
