  ...
  <div class="my-5 grid grid-cols-1 md:grid-cols-3">
    <div class="col-span-1 w-full md:border-r-4 md:border-r-transparent mb-5" >
      ...
      <!--<p :class="{'text-2xl': true, 'px-5': true, 'py-2': true, 'text-white': true, 'hover:bg-slate-800': true, 'border-4': true, 'border-transparent': true, 'hover:text-teal-300': true, 'cursor-pointer': true, 'select-none': true, 'text-teal-300': state.page === key, 'bg-slate-800': state.page === key, 'border-cyan-500': state.page === key}" v-for="(value, key) in SettingsList" :key="key" @click="state.page = key">
        {{ value }}</p>-->
      <p class="text-2xl px-5 py-2 border-4 border-cyan-500 cursor-pointer select-none hover:bg-slate-100">用户</p>
    </div>
    <div class="md:col-start-2 md:col-span-2 w-full md:border-l-4 md:border-l-transparent px-10">
        ...
        <div class="col-start-5 col-span-7 p-5 divide-gray-900">
          <div v-if="!state.editMode">
            <div class="flex justify-between items-center mb-2">
              <span :class="{'text-2xl': true, 'animate-pulse': !userInfo.display_name, 'bg-slate-300': !userInfo.display_name, 'h-8': !userInfo.screen_name, 'w-32': !userInfo.display_name,}">{{ userInfo.display_name ? userInfo.display_name : '' }}</span>
              <span class="cursor-pointer hover:bg-slate-700" @click="state.editMode = !state.editMode">
            ...
            <p :class="{'text-sm': true, 'animate-pulse': !userInfo.screen_name, 'bg-slate-300': !userInfo.screen_name, 'h-5': !userInfo.screen_name, 'w-15': !userInfo.screen_name,}">{{ userInfo.screen_name ? '@' + userInfo.screen_name : '' }}</p>
          </div>
          <div v-else class="grid grid-cols-12 items-center ">
            ...
            <div class="col-start-1 col-span-11 ">
              <input id="input-screen-name" type="text" class="form-input w-full caret-teal-300" v-model="screenName">
            </div>
          ...
          <div class="flex justify-between gap-5">
            <div @click="setPlatform('ns')" :class="{'cursor-pointer': true, 'w-full': true, 'py-2': true, 'px-5': true, 'bg-[#E60012]': Platform === 'ns', 'text-white': Platform === 'ns', 'border-2': true, 'border-[#E60012]': Platform === 'ps',}">Nintendo Switch</div>
            <div @click="setPlatform('ps')" :class="{'cursor-pointer': true, 'w-full': true, 'py-2': true, 'px-5': true, 'bg-[#0070D1]': Platform === 'ps', 'text-white': Platform === 'ps', 'border-2': true, 'border-[#0070D1]': Platform === 'ns',}">PlayStation</div>
          </div>
        </div>
      </div>
    </div>
...
const screenName = computed({
  ...
  set (value: string) {store.dispatch('setCoreValue', {key: 'screen_name', value})}
})
